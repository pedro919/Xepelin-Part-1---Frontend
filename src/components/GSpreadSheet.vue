...
const emit = defineEmits(['changedRate', 'changedEmail'])
</script>
            ...
            <table class="min-w-full divide-y divide-gray400">
              ...
              <tbody class="divide-y divide-gray-300 bg-white">
                <tr v-for="row in ratesList" :key="row.idOp" class="h-full">
                  ...
                  <td class="whitespace-nowrap px-3 text-sm text-gray-500 text-center h-full">
                    <input v-model="row.email" type="email" class="text-gray-900 text-center border-gray-900 border rounded-md w-full h-10" placeholder="Email" @change="$emit('changedEmail', $event, row.idOp, row.rate, row.email)">
                  </td>
                </tr>
              </tbody>
            </table>
